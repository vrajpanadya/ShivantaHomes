...
//                 <div><b>Call us</b><a href={`tel:${(settings.phone ?? '').replace(/\s/g, '')}`}>
//   {settings.phone ?? ''}
// </a></div>
//               </div>
//               <div className="contact-card" data-reveal>
...
export default function Home() {
  ...
  const stackItems = useMemo(() => {
    ...
    const featured = safeGallery.filter((g) => g.featured);
    const list = featured.length >= 2 ? featured : safeGallery.slice(0, 3);
    return list.slice(0, 4);
  }, [gallery]);
  ...
  const filtered = useMemo(() => {
    const safeGallery = Array.isArray(gallery) ? gallery : [];
    return cat === 'All' ? safeGallery : safeGallery.filter((g) => g.category === cat);
  }, [gallery, cat]);
  const go = (href: string) => (e: React.MouseEvent) => { e.preventDefault(); scrollToHash(href); };
