...
export default function Hero({ data, play, fallbackTitle = 'SHIVANTA HOMES' }: Props) {
  ...
  const primaryCta = {
    text: data?.primaryCta?.text ?? 'Book a Site Visit',
    href: data?.primaryCta?.href ?? '#contact',
  };
  const secondaryCta = {
    text: data?.secondaryCta?.text ?? 'Explore Residences',
    href: data?.secondaryCta?.href ?? '#residences',
  };
  // Heading is optional (admin can leave it empty — the photograph's own signboard carries the name).
  const lines = useMemo(() => {
    const heading = (data?.heading ?? '').trim();
  ...
  const labelText = (data?.label ?? '').trim(); // optional too — empty hides the location label (and its line)
  const bare = lines.length === 0 && !labelText; // nothing above the CTAs
  const traced = !videoUrl && !!posterUrl; // linework shows only if the photo registers to the traced render
  const go = (href: string) => (e: React.MouseEvent) => { e.preventDefault(); scrollToHash(href); };
    ...
    <section className={`hero${bare ? ' hero--bare' : ''}`} ref={root}>
      <div className="hero-bg" aria-hidden="true">
        <div className="hero-plate">
          <div className="hero-zoom">
            {videoUrl ? (
              <video src={videoUrl} poster={posterUrl || undefined} autoPlay muted loop playsInline />
            ) : posterUrl ? (
              <img src={posterUrl} alt="SHIVANTA HOMES entrance, Chavaj, Bharuch" decoding="async" />
            ) : null}
          </div>
        </div>
        {traced && (
      ...
      <div className={`container hero-content${bare ? ' hero-content--bare' : ''}`}>
        ...
        <div className="hero-ctas">
          <a className="btn btn-primary" href={primaryCta.href} onClick={go(primaryCta.href)}>
            {primaryCta.text} <Icon name="arrow" size={16} className="cta-arrow" />
          </a>
          <a className="btn btn-ghost on-dark" href={secondaryCta.href} onClick={go(secondaryCta.href)}>
            {secondaryCta.text}
          </a>
        </div>
      </div>
